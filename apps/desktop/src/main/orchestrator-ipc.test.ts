  ...
  it("registers lifecycle, workstream, and event handlers", async () => {
    ...
    const orchestrator = {
      ...
      startBuildAgentRun: vi.fn(() => ({
        ...
        goal: "Exercise IPC planning.",
        workspacePath: "/tmp/workspaces/ws-1/run-1",
        branchName: "mergepilot/ws-1/build/run-1",
        summary: "Build agent finished.",
    ...
    await expect(ipc.invoke("pull-requests:open", { workstreamId: "ws-1", agentRunId: "run-1" })).resolves.toMatchObject({
      id: "pr-1",
      prUrl: "https://github.com/ss-andrade/mergepilot/pull/42",
      status: "open"
    });
